...
def train_one_epoch(cfg, model, optimizer, data_loader, device, epoch, tfboard=None, softmax_criterion_s1=None, softmax_criterion_s2=None, softmax_criterion_s3=None):
    model.train()
    ...
    for i, (images, targets) in enumerate(
        ...
        loss_dict, feats_reid_1st, feats_reid_2nd, feats_reid_3rd, targets_reid = model(images, targets, epoch=epoch)
        if cfg.MODEL.LOSS.USE_SOFTMAX:
            softmax_loss_1st = cfg.SOLVER.LW_BOX_SOFTMAX_1ST * softmax_criterion_s1(feats_reid_1st, targets_reid)
            softmax_loss_2nd = cfg.SOLVER.LW_BOX_SOFTMAX_2ND * softmax_criterion_s2(feats_reid_2nd, targets_reid)
            softmax_loss_3rd = cfg.SOLVER.LW_BOX_SOFTMAX_3RD * softmax_criterion_s3(feats_reid_3rd, targets_reid)
            loss_dict.update(loss_box_softmax_1st=softmax_loss_1st)
