...
class OIMPlusLoss(nn.Module):
    ...
    def forward(self, inputs, roi_label, roi_ious, epoch=None):
        ...
        projected = torch.log(torch.softmax(projected, dim=1) * self.reliability)

        loss_oim = F.nll_loss(projected, label, ignore_index=5554)
