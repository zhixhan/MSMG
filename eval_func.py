...
def eval_detection(
    ...
    for anno, det in zip(annos, gallery_dets):
        ...
        if num_det == 0:
            ...
            continue
        '''
        imgopen = cv2.imread(anno['img_path'])
        imgname = anno['img_name']
        for ind in range(num_det):
            d = det[ind]
            cv2.rectangle(imgopen, (int(d[0]), int(d[1])), (int(d[2]), int(d[3])), (0,255,0),2)
            cv2.putText(imgopen, str(round(d[4],2)), (int(d[0]), int(d[1])-2),cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0,255,0),2)
        cv2.imwrite('./logs/visual_noclip/'+imgname, imgopen)
        '''
...
def eval_search_prw(
    ...
    for i in range(len(query_dataset)):
        ...
        #if (query_roi[0][3]-query_roi[0][1]) * (query_roi[0][2]-query_roi[0][0]) <= 5000:
        #    continue
