...
from models.transhead_hat3_3da_cat import HATHead
from models.visroi import visual_roi
...
class SeqNet(nn.Module):
    ...
    def inference(self, images, targets=None, query_img_as_gallery=False):
        ...
        if targets is not None and not query_img_as_gallery:
            ...
            box_features, _, _, feat4 = self.roi_heads.reid_head(box_features)
            visual_roi(ori_targets, feat4)
...
class SeqRoIHeads(RoIHeads):
    ...
    def forward(self, features, proposals, image_shapes, targets=None, query_img_as_gallery=False, epoch=None):
        ...
        if not self.training and query_img_as_gallery:
            ...
            gt_box_features, _ = self.reid_head(gt_box_features)
        ...
        box_features, location_features = self.reid_head(box_features)
        box_regs = self.box_predictor(F.adaptive_max_pool2d(location_features, 1))
