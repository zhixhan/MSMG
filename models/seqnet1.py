...
from models.pvtv2 import build_pvtv2
from models.transhead_hat1 import HATHead

class SeqNet(nn.Module):
    def __init__(self, cfg):
        super(SeqNet, self).__init__()

        backbone, box_head = build_pvtv2(pretrained=True)
        ...
        self.lw_box_reid = cfg.SOLVER.LW_BOX_REID_3RD
    ...
    def inference(self, images, targets=None, query_img_as_gallery=False):
        ...
        if targets is not None and not query_img_as_gallery:
            ...
            _, _, box_features = self.roi_heads.reid_head(box_features)
            ...
            detections, _, _, _ = self.roi_heads(
    ...
    def forward(self, images, targets=None, query_img_as_gallery=False, epoch=None):
        ...
        _, detector_losses, feats_reid, targets_reid = self.roi_heads(features, proposals, images.image_sizes, targets, epoch=epoch)
        ...
        losses["loss_rcnn_reid"] *= self.lw_box_reid
        return losses, feats_reid, targets_reid
...
class SeqRoIHeads(RoIHeads):
    def __init__(
        ...
        **kwargs
        ...
        self.reid_loss = reid_loss(oim_dim, num_pids, num_cq_size, oim_momentum, oim_scalar, max_epoch = max_epoch)
    ...
    def forward(self, features, proposals, image_shapes, targets=None, query_img_as_gallery=False, epoch=None):
        ...
        if not self.training and query_img_as_gallery:
            ...
            _, _, gt_box_features = self.reid_head(gt_box_features)
            embeddings = self.embedding_head(gt_box_features)
        ...
        if boxes[0].shape[0] == 0:
            ...
            return [dict(boxes=boxes, labels=labels, scores=scores, embeddings=embeddings)], [], None, None

        # --------------------- Baseline head -------------------- #
        box_features = OrderedDict()
        for fname, box_roi_pooling in self.box_roi_pool.items():
            box_features[fname] = box_roi_pooling(features, boxes, image_shapes)
        _, _, box_features = self.reid_head(box_features)
       
        box_embeddings = self.embedding_head(box_features)

        result, losses = [], {}
        feats_reid, targets_reid = None, None
        if self.training:
            ...
            loss_rcnn_reid, feats_reid, targets_reid = self.reid_loss(box_embeddings, box_pid_labels, boxes_ious, epoch=epoch)
            losses.update(loss_rcnn_reid=loss_rcnn_reid)
        ...
        return result, losses, feats_reid, targets_reid
