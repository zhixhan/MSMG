...
class DecoderBlock2(nn.Module):
    ...
    def forward(self, part_querys, x, H, W):
        B, C, H, W = x.size()
        x = x.view(B, C, -1).permute(0, 2, 1)
        part_querys = part_querys + self.drop_path(self.norm0(self.attn(part_querys)))
        part_feats = self.rpda(part_querys, x, H, W, ra_type='gar', offset_range_factor=(2,7)) 
...
class HATHead(nn.Module):
    ...
    def __init__(self, feature_names=["feat_res2", "feat_res3", "feat_res4"], in_channels=[256, 512, 1024], depth=2, num_heads=8, embed_dim=384, sr_ratios=[1, 1, 1], spacial_size=14, part_feats=7):
        ...
        self.decoder1 = DecoderBlock(dim=embed_dim, num_heads=num_heads, sr_ratio=1, spacial_size=spacial_size[-1])
        self.decoder2 = DecoderBlock2(dim=embed_dim, num_heads=num_heads, sr_ratio=1, spacial_size=spacial_size[-1])
        self.decoder3 = DecoderBlock2(dim=embed_dim, num_heads=num_heads, sr_ratio=1, spacial_size=spacial_size[-1])
    ...
    def forward(self, x):
        ...
        part_feat2 = self.decoder1(feat2, H, W)
        part_feat3 = self.decoder2(part_feat2, feat3, H, W)
        part_feat4 = self.decoder3(part_feat3, feat4, H, W)
