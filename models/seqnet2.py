...
from models.transhead_hat2_2da_sum_share import HATHead
...
class SeqNet(nn.Module):
    ...
    def inference(self, images, targets=None, query_img_as_gallery=False):
        ...
        if targets is not None and not query_img_as_gallery:
            ...
            _, box_features_2nd, box_features_3rd = self.roi_heads.reid_head(box_features)
            ...
            detections, _, _, _, _ = self.roi_heads(
    ...
    def forward(self, images, targets=None, query_img_as_gallery=False, epoch=None):
        ...
        _, detector_losses, feats_reid_2nd, feats_reid_3rd, targets_reid = self.roi_heads(features, proposals, images.image_sizes, targets, epoch=epoch)
        ...
        return losses, feats_reid_2nd, feats_reid_3rd, targets_reid
...
class SeqRoIHeads(RoIHeads):
    def __init__(
        ...
        **kwargs
        ...
        self.embedding_head_2nd = embedding_head
        self.embedding_head_3rd = deepcopy(self.embedding_head_2nd)
        self.oim_dim = oim_dim
        self.reid_loss_2nd = reid_loss(oim_dim, num_pids, num_cq_size, oim_momentum, oim_scalar, max_epoch = max_epoch)
    ...
    def forward(self, features, proposals, image_shapes, targets=None, query_img_as_gallery=False, epoch=None):
        ...
        if not self.training and query_img_as_gallery:
            ...
            _, gt_box_features_2nd, gt_box_features_3rd = self.reid_head(gt_box_features)
        ...
        if boxes[0].shape[0] == 0:
            ...
            return [dict(boxes=boxes, labels=labels, scores=scores, embeddings=embeddings)], [], None, None, None

        # --------------------- Baseline head -------------------- #
        box_features = OrderedDict()
        for fname, box_roi_pooling in self.box_roi_pool.items():
            box_features[fname] = box_roi_pooling(features, boxes, image_shapes)
        _, box_features_2nd, box_features_3rd = self.reid_head(box_features)
       
        box_embeddings_2nd = self.embedding_head_2nd(box_features_2nd)
        box_embeddings_3rd = self.embedding_head_3rd(box_features_3rd)

        result, losses = [], {}
        feats_reid_2nd, feats_reid_3rd, targets_reid = None, None, None
        if self.training:
            ...
            loss_rcnn_reid_2nd, feats_reid_2nd, targets_reid = self.reid_loss_2nd(box_embeddings_2nd, box_pid_labels, boxes_ious, epoch=epoch)
        ...
        return result, losses, feats_reid_2nd, feats_reid_3rd, targets_reid
