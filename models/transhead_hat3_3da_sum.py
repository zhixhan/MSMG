...
class HATHead(nn.Module):
    ...
    def __init__(self, feature_names=["feat_res2", "feat_res3", "feat_res4"], in_channels=[256, 512, 1024], depth=2, num_heads=8, embed_dim=384, sr_ratios=[1, 1, 1], spacial_size=14, part_feats=7):
        ...
        self.decoder2 = DecoderBlock(dim=embed_dim, num_heads=num_heads, sr_ratio=1, spacial_size=spacial_size[-1])
        self.decoder3 = DecoderBlock(dim=embed_dim, num_heads=num_heads, sr_ratio=1, spacial_size=spacial_size[-1])
    ...
    def forward(self, x):
        ...
        part_feat3 = self.decoder2(feat3, H, W)
        part_feat4 = self.decoder3(feat4, H, W)
