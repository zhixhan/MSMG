...
class HATHead(nn.Module):
    ...
    def __init__(self, feature_names=["feat_res2", "feat_res3", "feat_res4"], in_channels=[256, 512, 1024], depth=2, num_heads=8, embed_dim=384, sr_ratios=[1, 1, 1], spacial_size=14, part_feats=7):
        ...
        self.output = nn.ModuleDict([
            ['feat_res2', nn.Conv2d(embed_dim, 2048, kernel_size=1, stride=1, bias=False)],
            ['feat_res3', nn.Conv2d(embed_dim, 2048, kernel_size=1, stride=1, bias=False)],
    ...
    def forward(self, x):
        ...
        part_feat2 = torch.chunk(feat2, self.part_feats, dim=-2)
        for i, pf in enumerate(part_feat2):
            feats2.append(["part_trans_"+str(i), F.adaptive_max_pool2d(pf, 1)])
        feats2.append(["trans_feat", F.adaptive_max_pool2d(feat2, 1)])
        
        feat3 = self.output[res3](feat3)
        part_feat3 = torch.chunk(feat3, self.part_feats, dim=-2)
        for i, pf in enumerate(part_feat3):
            feats3.append(["part_trans_"+str(i), F.adaptive_max_pool2d(pf, 1)])
        feats3.append(["trans_feat", F.adaptive_max_pool2d(feat3, 1)])
        
        feat4 = self.output[res4](feat4)
        part_feat4 = torch.chunk(feat4, self.part_feats, dim=-2)
        for i, pf in enumerate(part_feat4):
            feats4.append(["part_trans_"+str(i), F.adaptive_max_pool2d(pf, 1)])
        feats4.append(["trans_feat", F.adaptive_max_pool2d(feat4, 1)])
